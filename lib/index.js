...
exports.test = ({ name, unit, mock, spec }) => {
  ...
  const runTest = async (options = {}) => {
    ...
    try {
      // assume it is a spec object if not a function
      if (typeof spec !== 'function') {
        const tests = Array.isArray(spec) ? spec : [spec];
        spec = async (u) => {
          return Promise.all(tests.map(async (t, i) => {
            const o = await u(...t.given);
            // TODO: adding a name here ruins the nice error message from deep equal
            // I should look into using the props for better assertion error messaging
            await exports.assert.deepEqual(o, t.output);
          }));
        };
      }
