...
exports.test = ({ name, unit, mock, spec }) => {
  const globalName = '__M0K__' + mockId++;
  ...
  const runTest = async (options = {}) => {
    const resolve = (req) => {
      ...
      return require.resolve(path.resolve(path.dirname(site.getFileName()), req));
    };
    ...
    const bustCached = () => {
      ...
      if (mock) {
        mock.forEach((m) => {
          delete require.cache[resolve(m[0])];
        });
      } 
    };
    ...
    const removeHook = pirates.addHook((_, filename) => {
    ...
    bustCached();
    const injectedUnit = require(resolve(unit));
    const injectedMocks = mock && mock.map(mock => require(resolve(mock[0])));
    bustCached();
    removeHook();

    let result = {
      name: name || spec.name, // use a the test name function as a backup if available
      site: site,
